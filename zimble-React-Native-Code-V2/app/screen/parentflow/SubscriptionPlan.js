...
import {changeSubscription, educationFavourite, getEducationList, getChatHistory, getNotificationCount ,getSubscription} from '../../api';
...
class SubscriptionPlan extends React.Component {
    ...
    componentWillMount() {
        ...
        console.log(navigation, route.params.planData)
        this.setState({ 
            chatReq: this.props.childchatreq,
            planData:route.params.planData
        })
    ...
    async getSubscriptionData() {
        ...
        try {
            ...
            if (result && result.status == 'success') {
                ...
                for (let index = 0; index < result.details.data.length; index++) {
                    console.log(JSON.stringify(result.details.data[index]))
                    var element = result.details.data[index];
                    element.planId=element.id
    ...
    _renderItemWithParallax = ({ item, index }, parallaxProps) => {
        var data = item
        console.log(data,this.state.planData.planId)
        var even = (index) % 3
        var backgroundColor = [
            colors.Greenish,
            colors.Purple,
        ...
        var id=data.id?data.id:data.planId
        console.log(id,this.state.planData.planId==id)
        return (
    ...
    async onSubscribe(data){
        ...
        try {
            ...
            let reqData={
                ...
                planId:data.id?data.id:data.planId,
                // cardId:this.state.selectedCard._id
            }
            // setLoader(true)
            const result = await changeSubscription(reqData);

            if (result && result.status == 'success') {
                let message = {}
                message.message = 'Subscription Changed successfully'
