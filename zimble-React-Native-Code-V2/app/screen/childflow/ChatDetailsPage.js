// Child will be able to view the chats with the users able to reply on the chats.

// Create Event: Child will be able to create and send event directly from messages as well. User will be redirected to the Create Event Screen.

// Note: Each event that will be created by the child will be updated shown in events session as well as shared over the messages in both the scenarios either created from events sessions or from the messages.
...
import { ButtonRightIcon} from '../../components/Button'
import { getChatHistory, API_BASE_URL_SOCKET } from '../../api';
...
class ChatDetailsPage extends React.PureComponent {
    ...
    constructor(props) {
        ...
        this.socket.on('connect', () => {
            this.socket.emit('room join', {
                senderId:this.props.childchatreq.childId,
                roomId:this.props.childchatreq.childId
            })
            this.socket.on('message', (r) => {
                
                if(r.receiverId==this.state.chatReq.childId){
    ...
    SentMessage(msg) {
     ...
     EventMesage (msg) {
        return (
            <View style={{ minHeight: 100, marginTop: 20, width: width - 40, backgroundColor: colors.white, borderRadius: 10,justifyContent:'center' }}>
                <View style={{ flexDirection: 'row', }}>
                    ...
                    <View style={{ marginHorizontal: 0, width: width - 140, marginTop: 10 }}>
                    ...
                    <View style={{ justifyContent: 'center', backgroundColor: colors.white, borderBottomWidth: 1, borderBottomColor: colors.charcolColorNew, }}>
                        ...
                        <View style={{ flexDirection: 'row', marginLeft: 20, paddingVertical: 10 }}>
                            ...
                            <View style={{ 
                                ...
                                <Image source={{ uri:this.state.chatReq.parentProfile }} style={{ height: 58, width: 58, borderRadius: 29,  alignSelf: 'center' }}></Image>
                            </View>
                            <View style={{ marginHorizontal: 0, width: width - 40, marginTop: 10 }}>
                                <Text style={[styles.robotoBoldText, { marginHorizontal: 10, fontSize: 17, color: colors.allowTitle, marginRight: 40 }]}>{this.state.chatReq.parentName} </Text>
                            ...
                            return (
                                ...
                                <View onPress={() => onSelectChild(item)} style={[{ minHeight: 60, width: width - 40, borderRadius: 8, alignItems: 'center', marginLeft: 20, marginRight: 0, marginBottom: 15,  }]}>
                                    {item.messageType != 'text' ?
                                        ...
                                        :
                                        item.senderId == this.state.chatReq.childId ?
                    ...
                    <KeyboardAvoidingView>
                        <View style={{ flexDirection: 'row', paddingBottom: 15, borderBottomColor: colors.charcolColorNew, borderBottomWidth: 1,  }}>
                            <ButtonRightIcon
                                ...
                                borderRadius={30}
                                marginTop={15}
                                palceholder="Create Event"
                                type="event"
                                palceholderColor={colors.Greenish}
                                onAction={() => {this.props.navigation.navigate('NewEventRequest') }}
                        ...
                        <View style={[styles.cardChatText], { backgroundColor: colors.white, width: width - 40, marginLeft: 20, height: 60, marginTop: 10, flexDirection: 'row', borderRadius: 10, marginBottom: 10 }}>
                            ...
                            <TouchableOpacity disabled={!this.state.message} onPress={() => this.onSend()} style={{ width: 60, backgroundColor: colors.childblue, height: 60, justifyContent: 'center', alignItems: 'center', borderBottomEndRadius: 10, borderTopEndRadius: 10 }}>
                                ...
                                {/* <Text style={[styles.robotoRegularText, { fontSize: 15, color: colors.titleText, }]}> Send </Text> */}
                            </TouchableOpacity>
                        </View>
                    </KeyboardAvoidingView>
                </View>
