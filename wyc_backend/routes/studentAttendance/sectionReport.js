...
function getFinalResult(workingDaysInfo, studentInfo, stuAttendance, holiday, weekOff, min_attendance, callback) {
    ...
    const monthReport = workingDaysInfo.map((row) => {
        return {
            month: row.month,
            totalWorkingDays: row.totalWorkingDays,
            monthWorkingDays: row.monthWorkingDays,
            monthNotTakenDays: row.monthNotTakenDays
        };
    });
    ...
    return callback({ myArray, monthReport, studentCount, not_meet_min_attendance, avg_attendance });
}
