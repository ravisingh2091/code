...
function add(req, res, next) {
    ...
    ExtraCollectionBoucherPayment.create(req.body).then((result) => {
        if (result) {
               console.log(result.id)
            let boucher_id=dateNew+result.id
            let paymentData=req.body.items.map(it=>{
                it.payment_boucher_id=result.id	
                it.branch_id=req.query.branch_id
                return it
            })
            console.log(boucher_id)
            ExtraCollectionBoucherPayment.update({ boucher_id: boucher_id}, { where: { id: result.id } }).then(data=>{
                console.log(data)
            },error=>{
                console.log(error)
            })
            return ExtraCollectionPayment.bulkCreate(paymentData, {returning: true}).then(() => {
                res.json({
                    status: true,
...
//     ExtraCollectionPayment.bulkCreate(req.body.paymentData, {returning: true}).then((result) => {
//         if (result) {
//             res.json({
//                 status: true,
//                 message: 'Payment added successfully',
